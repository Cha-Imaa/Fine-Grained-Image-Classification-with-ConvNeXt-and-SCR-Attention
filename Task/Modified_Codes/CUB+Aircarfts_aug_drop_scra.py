...
import wandb
wandb.init(project="CV703_task2_aug_color_dropout_seb")
...
PATH_CHECKPOINT_FOLDER = f'{RESULTS}/Checkpoints/Task2_aug_color_dropout_seb'
PATH_LOGGINGS_FOLDER = f'{RESULTS}/loggings/Task2_aug_color_dropout_seb'
PATH_PICKLE_FOLDER = f'{RESULTS}/pickels/Task2_aug_color_dropout_seb'
...
test_dataset_aircraft = FGVCAircraft(root=f"{PATH_AIRCRAFT_DATASET}", transform=test_data_transforms, train=False)
...
# Function to store initial pretrained weights
...
def train(model, concat_loader_train, criterion, optimizer, scheduler):
    ...
    for batch in concat_loader_train:
        ...
        # Print status every 225 batches
...
# Set up logging
log_filename = os.path.join(PATH_LOGGINGS_FOLDER, 'dropout_logfile.log')
...
for epoch in range(num_epochs):
    ...
    if val_accuracy > best_val_accuracy:
        ...
        checkpoint_path = os.path.join(PATH_CHECKPOINT_FOLDER, 'dropout_best_weights.pth')
...
filename =  os.pah.join(PATH_PICKLE_FOLDER,'dropout_history.pickle')
...
# Assuming 'model2' is your PyTorch model
